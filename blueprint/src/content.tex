...
% Gacs source line 901
 \begin{definition}
  \label{def:fixed-U}
  \lean{Kolmogorov.isOptimalConditional, Kolmogorov.existsIsOptimalConditional}
  \leanok
...
\( C(x \mvert y) \) for \( C_U(x \mvert y) \). 
 \end{definition}
\begin{formalization}
No machine is fixed. Every result takes the interpreter $U$ and the hypothesis \texttt{isOptimalConditional U} as parameters, and \texttt{existsIsOptimalConditional} (Theorem~\ref{thm:invar}) supplies one.
\end{formalization}
...
% Gacs source line 919
 \begin{definition}
  \label{def:tuples}
  \lean{Kolmogorov.plainK, Kolmogorov.listCode}
  \leanok
  \uses{def:fixed-U}
...
We therefore define 
 ...
 \end{definition}
\begin{formalization}
$U(p) = U(p,\Lambda)$ is the empty context of \texttt{plainK}; tuples are coded by \texttt{listCode}.
\end{formalization}
...
The relation \( f\eqm g \) means \( f \lem g \) and \( f \gem g \).
 \end{definition}
\begin{formalization}
No notation is introduced in Lean. Every statement carries its constant explicitly, in the shape $\exists c,\ \forall x,\ f(x) \le g(x) + c$, and multiplicative bounds are written in $[0,\infty]$ as $2^{-c} \cdot f \le g$.
\end{formalization}
...
\begin{proof}
  \leanok
  \uses{thm:upbd}
  Only finitely many objects have complexity at most $k$, since there are fewer than $2^{k+1}$ programs of length at most $k$; so $C(n) \to \infty$.
\end{proof}
...
Let us elaborate on the upper bound of Theorem~\ref{thm:upbd}. For any string
\( x \in \dS_{r} \) of length \( n \), we have
\begin{equation}  \label{eq:binupbd}
C(x) \lea n\log r + 2 \log r.
\end{equation}
In particular, for any binary string \( x \), we have \( C(x) \lea \len{x} \).
...
% Gacs source line 1089
 \begin{definition}
  ...
  \lean{Kolmogorov.pairCode, Kolmogorov.KPPair}
  \leanok
...
The definition of conditional complexity is extended to pairs, etc. by 
 ...
 \end{definition}
\begin{formalization}
Pairs are coded by the prefix-free \texttt{pairCode}, longer tuples by \texttt{listCode}; a complexity of a tuple is the complexity of its code, so $C(x,y)$ is \texttt{plainK U (pairCode x y)} and \texttt{KPPair} is the prefix case.
\end{formalization}
...
\begin{proof}
  \leanok
  \uses{thm:rectransf, def:pair-complexity}
  Each inequality is Theorem~\ref{thm:rectransf} applied to a suitable p.r.\ function: $U_p$ (and, for the two-sided bound, its one-to-one inverse), and the pairing and projection maps of Definition~\ref{def:pair-complexity}.
\end{proof}
...
\begin{proof}
  \leanok
  \uses{thm:rectransf, thm:addit}
  Theorem~\ref{thm:rectransf} for $U_p$ composed with the pairing map bounds $C(U_p(x,y))$ by $C(x,y) + J(\len{p})$; Theorem~\ref{thm:addit} bounds $C(x,y)$.
\end{proof}


This corollary implies the following continuity property of the function \( C(n) \):
for any natural numbers \( n,h \), we have
\begin{equation}
|C(n+h)-C(n)| \lea J(\log h). \label{eq:continui}
\end{equation}
Indeed, \( n+h \) is a recursive function of \( n \) and \( h \).
...
\begin{proof}
  \leanok
  \uses{def:pair-complexity}
  Let $q$ be a shortest program for $y$ given $\ang{x,i}$, so $\len{q} = C(y \mvert x,i)$. From $x$, the number $i - \len{q}$ and $q$ itself one recovers $i = (i - \len{q}) + \len{q}$ and then $y$. So $q$ serves, up to a constant, as a description of $y$ given $\ang{x,\, i - C(y \mvert x,i)}$.
\end{proof}
...
a G\"odel number of \( g \) if \( g=S_{p}=-S_{q} \).
 \end{definition}
\begin{formalization}
The universal lower semicomputable function $S_p$ and its G\"odel numbers are not formalised as such. The library enumerates lower semicomputable semimeasure approximations instead (\texttt{IsLSC}, \texttt{lscEnum}), which is all Chapter~1 uses of them; no formalised result depends on this definition or on Definition~\ref{def:universal-lsc}.
\end{formalization}
...
does not contain any infinite computably enumerable subsets. 
 \end{definition}
\begin{formalization}
Not formalised; no formalised result depends on it.
\end{formalization}
...
% Gacs source line 1547
 \begin{definition}
  ...
  \lean{Kolmogorov.IsPrefixFree, Kolmogorov.IsPrefixMachine}
  \leanok
...
% Gacs source line 1826
 \begin{definition}
  \label{def:code}
  \lean{Kolmogorov.Map, Kolmogorov.IsPrefixFree}
  \leanok
  \uses{def:prefix-free}
...
A code is a \df{prefix code} if \( E \) is a prefix-free set.
 \end{definition}
\begin{formalization}
A code is a \texttt{Map}, read as its decoding function; it is a prefix code when its domain \texttt{IsPrefixFree}.
\end{formalization}
...
\begin{formalization}
Formalized in the multiplicative form $2^{-n} f(x,n) \eqm \m(x,n)$ that the proof below actually establishes, in $[0,\infty]$ rather than through logarithms. The two halves are separate results; the lower half is stated for $n \ge K(x) + c$, the printed hypothesis shifted by a constant that depends only on $T$.
\end{formalization}
...
\begin{proof}
  \leanok
  \uses{thm:Hstat}
  Take $n = K(x)$ in the upper half of Theorem~\ref{thm:Hstat}, which carries no hypothesis: $f(x,K(x)) \lem 2^{K(x)}\, \m(x,K(x)) \lem 2^{K(x) - K(x,K(x))} \lem 1$, since $K(x) \lea K(x,K(x))$.
\end{proof}
...
\begin{formalization}
The padding step needs $l \ge 0$, that is $n \ge K(x,n) + c_0$ with $c_0 = r + \len{\bg(r)^o}$, which exceeds the stated $n \ge K(x)$ by up to $\log n$. It is formalized as a separate lemma under that hypothesis. The window $K(x) + c \le n < K(x,n) + c_0$ is closed by a second machine $G'(1^k 0\, p) = T(p)$, which pads with a self-delimiting filler instead of stating a length: universality gives a tag $g'$ with $T(g' 1^k 0\, q) = x$ for a shortest program $q$ of $x$ and every $k$, so $f(x,n) \ge 1$ for all $n \ge K(x) + \len{g'} + 1$, and in the window a single program already gives $2^{-n} f(x,n) \ge 2^{-n} > 2^{-K(x,n) - c_0} \gem \m(x,n)$.
\end{formalization}

% Gacs source line 2089
 \begin{definition}
  \label{def:Dn}
  \lean{Kolmogorov.haltSet, Kolmogorov.haltCount, Kolmogorov.windowCount}
  \leanok
  \uses{def:K}
...
What can be said about the complexity of a binary string of length \( n \)? The
estimate \( K(x) \lea n + 2\log n \) was mentioned earlier; the same argument gives the
stronger estimate
\begin{equation} K(x) \lea \len{x} + K(\len{x}). \label{eq:monbd}
\end{equation}
In Lean this is \texttt{KPPlain\_le\_length\_add\_KPPlain\_length}, and it is the bound that
Theorem~\ref{thm:monbd} below sharpens into an equality for the monotone majorant.
...
% Gacs source line 2241
 \begin{definition}
  ...
  \lean{Kolmogorov.evalFuel, Kolmogorov.KFuel, Kolmogorov.trivBound}
  \leanok
  \uses{def:K}
...
optimal s.d.~interpreter $T$, and let us write 
 ...
 \end{definition}
\begin{formalization}
Not formalised, and not needed. The Lean proof of Theorem~\ref{thm:infsharp} bounds its search by a fuel value that is computable outright, by \texttt{Computable.natFind} over finitely many halting candidates, so no assumption about simulation overhead between machines enters; see the module docstring of \texttt{Solovay.lean}.
\end{formalization}


With these definitions, for each self-delimiting Turing machine \( \cM \) there are
constants \( m_{0},m_{1} \) with
\begin{equation}  \label{eq:fastopt}
K(n,m_{0}t) \le K_{\cM}(n,t) + m_{1}.
\end{equation}
The function \( K(n;t) \) is nonincreasing in \( t \).
